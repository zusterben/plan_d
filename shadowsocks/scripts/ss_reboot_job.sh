...
check_ip_now(){
	...
	if [ "$ss_lb_enable" == "1" ] && [ `dbus get ss_basic_server | grep -o "127.0.0.1"` ] && [ `dbus get ss_basic_port` == `dbus get ss_lb_port` ];then
		#负载均衡模式下检查/jffs/softcenter/configs/haproxy.cfg
		logger "【科学上网插件触发重启功能】========================================================"
		logger "【科学上网插件触发重启功能】：当前处于负载均衡状态，使用DNS:$(__get_server_resolver):$(__get_server_resolver_port)检查负载均衡节点IP是否更换..."
		if [ -f /jffs/softcenter/etc/haproxy.cfg ] && [ -n `pidof haproxy` ];then
			SERVER_INFO=$(cat /jffs/softcenter/etc/haproxy.cfg | grep -E "^\s\s\s\sserver"|sed 's/:/ /g'|awk '{print $2}')
			ADDR_INFO=$(cat /jffs/softcenter/etc/haproxy.cfg | grep -E "^\s\s\s\sserver"|sed 's/:/ /g'|awk '{print $4}')
			INFO_LINE=$(cat /jffs/softcenter/etc/haproxy.cfg | grep -E "^\s\s\s\sserver"|sed 's/:/ /g'|awk '{print $2" "$4}'|wc -l)
			local i=1
			local j=1
