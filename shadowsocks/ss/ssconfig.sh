...
start_dns() {
	...
	if [ "$ss_foreign_dns" == "2" ]; then
		...
		chinadns -p $DNSF_PORT -s $ss_chinadns_user -e $clinet_ip,$ss_real_server_ip -c /jffs/softcenter/ss/rules/chnroute.txt >/dev/null 2>&1 &
	fi
	...
	if [ "$ss_foreign_dns" == "5" ]; then
		...
		if [ "$ss_dns_china" == "13" -a "$ss_foreign_dns" == "5" ]; then
			...
			chinadns1 -p $DNSF_PORT -s $CDN,127.0.0.1:1055 -d -c /jffs/softcenter/ss/rules/chnroute.txt >/dev/null 2>&1 &
		fi
	fi
	#start chinadns_ng
	...
	if [ "$ss_dns_china" == "13" ] && [ "$ss_foreign_dns" == "9" ]; then
		...
		#	sed 's/# force-AAAA-SOA yes/force-AAAA-SOA yes/g' /jffs/softcenter/ss/rules/smartdns_template.conf > /tmp/smartdns.conf
		#	sed -i '/^#/d /^$/d' /tmp/smartdns.conf
		#else
			sed '/^#/d /^$/d' /jffs/softcenter/ss/rules/smartdns_template.conf > /tmp/smartdns.conf
		#fi
		...
		#else
			sed '/^#/d /^$/d /foreign/d' /jffs/softcenter/ss/rules/smartdns_template.conf > /tmp/smartdns.conf
		#fi
		smartdns -c /tmp/smartdns.conf >/dev/null 2>&1 &
	elif [ "$ss_dns_china" != "13" ] && [ "$ss_foreign_dns" == "9" ]; then
		...
		#else
			sed '/^#/d /^$/d /china/d' /jffs/softcenter/ss/rules/smartdns_template.conf > /tmp/smartdns.conf
		#fi
		smartdns -c /tmp/smartdns.conf >/dev/null 2>&1 &
	fi
	...
	if [ "$ss_basic_mode" == "6" ]; then
		...
		if [ "$ss_basic_mode" == "1" -a -z "$chn_on" -a -z "$all_on" ] || [ "$ss_basic_mode" == "6" ]; then
			...
			if [ "$ss_foreign_dns" == "2" ] || [ "$ss_foreign_dns" == "5" -a "$ss_dns_china" != "13" ] || [ "$ss_foreign_dns" == "10" ]; then
				...
				cat /jffs/softcenter/ss/rules/cdn.txt | sed "s/^/server=&\/./g" | sed "s/$/\/&$CDN#$DNSC_PORT/g" | sort | awk '{if ($0!=line) print;line=$0}' >>/tmp/sscdn.conf
			fi
		fi
	fi
